...
class BladeIMU:
    """
    A class to process IMU data mounted on a wind turbine blade.
    ...
    def decompose_accelerations(acc_mps2, acc_peaks, acc_mins):
        """Decomposes accelerometer signal into centripetal force and gravity vector."""
        acc_centrip = (acc_peaks + acc_mins) / 2.0
        acc_g = acc_mps2 - acc_centrip

        return acc_centrip, acc_g

    @staticmethod
    def get_acc_peaks_mins(
        signal,
        time,
        number_of_samples,
        height=None,
    ...
    def compute_peaks_mins(self):
        ...
        for i in range(3):
            ...
            if i == 2:
                ...
                detect = self.get_acc_peaks_mins(
                    self.acc_mps2[i], self.time, self.number_of_samples, distance=distance, width=30, prominence=10
                )
    ...
    @staticmethod
    def estimate_precone(acc_mps2, gyr_rps):
        """
        ...
        precone = (np.arctan2(acc_mps2[2], acc_mps2[1]) * 180 / np.pi).mean()  # mean of precone angle in degrees
        return precone
    ...
    def calculate_deflection(acc_mps2, gyr_rps):
        ...
        deflection = 180 - (
            np.arctan2(acc_mps2[2].mean(), acc_mps2[1].mean()) * 180 / np.pi
        )  # mean of deflection angle in degrees
        return deflection
    ...
    def calculate_azimuth(self):
        ...
        azimuth = np.array(
            [np.arctan2(ag_x[i], ag_y[i]) * 180 / np.pi for i in range(self.number_of_samples)]
    ...
    def blade_velocity(self, acc_peak_locations, acc_min_locations):
        ...
        w_hat_idx = np.concatenate(
            [acc_peak_locations[1], acc_min_locations[1]]
        )  # Get peak locations from all axes (x,y,z)
        w_hat_idx = w_hat_idx.astype("int")
        for i in range(1, len(w_hat_idx)):
            w_hat[w_hat_idx[i]] = (
                1.0 / (self.time[w_hat_idx[i]] - self.time[w_hat_idx[i - 1]]) * 2 * np.pi
            )  # w = 2*pi/T
...
class PostProcess:
    ...
    def process_imu(accelerometer_session, gyrometer_session, imu_coordinates, turbine_data):
        ...
        imu_data["height"] = imu.get_height(
            imu_data["azimuth"], hub_height=turbine_data["hub_height"], radius=imu_coordinates["r-coordinate"]
        )
